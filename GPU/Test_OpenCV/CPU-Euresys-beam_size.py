...
def preProcess_threshold(img_buffer, img_buffer_index):
    ...
    """
    _, img_buffer[img_buffer_index + 1] = cv2.threshold(img_buffer[img_buffer_index], 61, 255, cv2.THRESH_BINARY)


def preProcess_resize(img_buffer, img_buffer_index):
    """Resize the image using the OpenCV's resize function
    ...
    """
    img_buffer[img_buffer_index + 1] = cv2.resize(img_buffer[img_buffer_index], (1600, 846), interpolation=cv2.INTER_CUBIC)


def preProcess_canny(img_buffer, img_buffer_index):
    """Apply a canny contour detection to the image using the OpenCV's Canny function
    ...
    """
    img_buffer[img_buffer_index + 1] = cv2.Canny(img_buffer[img_buffer_index], 50.0, 100.0)
...
class Measure_size():
    ...
    def draw_axis(self, img, p_, q_, colour, scale):
        ...
        This is only for visualization for test purpose
    ...
    def get_orientation(self, pts, img):
        """Find the oriantation of the shape
    ...
    def process(self, img):
        ...
        for i, c in enumerate(contours):
            ...
            angle, center = self.get_orientation(c, BGR_img)
            PC_angle.append(angle)
            PC_center.append(center)

            PC1_angle = np.degrees(PC_angle[0])

            M = cv2.getRotationMatrix2D(image_center, PC1_angle, 1)
            rotated_image = cv2.warpAffine(img, M, (image_width, image_high))
            M = cv2.getRotationMatrix2D(image_center, 90, 1)
            rotated_image_90 = cv2.warpAffine(rotated_image, M, (image_width, image_high))
    ...
    def sum_pixels(self, img):
        ...
        for i, image in enumerate(img):
            histogram = []
            dst = cv2.reduce(image, 0, cv2.REDUCE_SUM, dtype=cv2.CV_32S)
...
class Process_and_plot():
    ...
    def update_plot(self, i):
        with Buffer(self.grabber, timeout=1000) as buffer:
            ...
            self.plot_list[0].set_data(self.img_buffer[0])
